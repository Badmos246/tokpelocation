...
const connexion = async () => {
  try {
    ...
    const response = await axios.get(
      `${base_url}/connexion?telephone=${Telephone.value}&password=${Password.value}`
    );
    router.push("/");
  } catch (error) {
...
<template>
  <layout>
    <div class="bg h-screen flex flex-col justify-center items-center">
      ...
      <form @submit.prevent="connexion" class="bg-white p-5 rounded-md">
        ...
        <input
          ...
          class="w-80 h-10 border-gray-500 border"
        /><br />
        ...
        <div class="w-full flex justify-center px-10 p-5 rounded-md">
          <input
            type="submit"
            value="SOUMETTRE"
            class="text-white w-30 h-12 bg-green-600"
          />
        </div>
      </form>
    </div>
